...
from modules.kommunicate.kommunicate_core import (
    send_text_message,
...
def send_button_with_urls(
    button_dict, button_title, session_id, open_link_in_new_tab="false"
):
    buttons = []
    for key, val in button_dict.items():
        buttons.append(
            KommActionableMessages.button_web_url(val, key, open_link_in_new_tab)
        )
...
def create_and_send_carousel(info_dict, session_id, message, url_title="More Info"):
    ...
    for title, value in info_dict.items():
        element_list.append(
            KommActionableMessages.carousel_payload(title, value[2], value[1], value[3])
        )
    # pprint(element_list)
...
def faq_dutyfree(session_id):
    try:
        buttons = [
            KommActionableMessages.card_link_button(
                "Order Now", "https://caribbeanairlinesdutyfree.com/"
            ),
            KommActionableMessages.card_link_button(
                "More Info",
                "https://www.caribbean-airlines.com/#/caribbean-experience/duty-free",
            ),
        ]
        img = "https://i.imgur.com/4PEsVdo.jpg"
        title = "CAL Duty Free"
        subtitle = "Browse. Order. Enjoy."
        kom_card_payload = json.dumps(
            [KommActionableMessages.card_payload(title, subtitle, img, buttons)]
        )
...
def faq_make_reservation(session_id):
    ...
    try:
        for key, val in reservation_dict.items():
            val.append(
                [KommActionableMessages.card_link_button("Call Representative", val[0])]
            )
        create_and_send_carousel(
            reservation_dict,
            session_id,
            message="You can contact our reservations team to create or make changes to your itinerary.",
        )
        website_url = {"Book a Flight": "https://www.caribbean-airlines.com/"}
