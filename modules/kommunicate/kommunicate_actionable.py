class KommActionableMessages:
    ...
    def quick_reply(title, message, reply_metadata=None):
        return {
            'title': title,
            'message': message
        } if reply_metadata is None else {
            'title': title,
            'message': message,
            'replyMetadata': reply_metadata
        }
    ...
    def list_button(action_type, name, link_or_text):
        return {
         ...
         } if action_type == 'quick_reply' else {
            "name": name,
            "action": {
                "type": "link",
                "url": link_or_text
    ...
    @staticmethod
    def card_payload(title, subtitle, img, buttons, overlay_text="", description="", title_ext=""):
        return {
            "title": title,
            "subtitle": subtitle,
            "header": {
    ...
    @staticmethod
    def carousel_payload(title, subtitle, img_src, buttons,  description="", overlay_text="", title_ext=""):
        return {
            "title": title,
            "subtitle": subtitle,
            "header": {
