...
def gen_fb_status_card(flight_resp, flight_status):
    ...
    message = AirlineFlightUpdate(
        "BW {flight_num} has unfortunately been {flight_status}".format(
            flight_num=flight_resp["flight_number"], flight_status=formatted_status
        ),
        flight_status,
        "en_US",
        "-",
        AirlineFlightUpdate.make_update_flight_info(
            ...
            AirlineFlightUpdate.make_airport(
                flight_resp["dept_code"], flight_resp["dept_city"]
            ),
            AirlineFlightUpdate.make_airport(
                flight_resp["arr_code"], flight_resp["arr_city"]
            ),
            AirlineFlightUpdate.make_flight_schedule(
                flight_resp["dept_time_local"], flight_resp["arr_time_local"]
            )
