...
class FBBot(object):
    def __init__(self, access_token, verify_token, **kwargs):
        ...
        self.request_endpoint_messenger_profile = "{0}/me/messenger_profile".format(
            self.graph_url
        )
    ...
    def appsecret_proof(self):
        ...
        hmac_object = hmac.new(
            str(self.app_secret), unicode(self.access_token), hashlib.sha256
        )
    ...
    def get_userinfo(self, user_id):
        resp = requests.get(
            "https://graph.facebook.com/v2.6/{0}?fields=first_name,last_name,profile_pic,locale,timezone,"
            "gender&access_token={1}".format(user_id, self.access_token)
        )
        return resp.json()
    ...
    def webhook(
        ...
        request,
        on_message_received,
        on_postback_received,
        on_account_linked=None,
        on_account_unlinked=None,
        on_location_received=None,
    ):
        """
        @required:
            request
        ...
        if request.method == "POST":
            ...
            for x in event:
                ...
                if x.get("message") and x["message"].get("text"):
                    message = (
                        "empty"
                        if (not ("message" in x) or not ("text" in x["message"]))
                        else x["message"]["text"]
                    )
                    on_message_received(sender_id, message, x)
                elif (
                    x.get("message")
                    and x["message"].get("attachments")
                    and x["message"]["attachments"][0].get("type")
                    and x["message"]["attachments"][0]["type"] == "location"
                ):
                    location = x["message"]["attachments"][0]["payload"]["coordinates"]
                    on_location_received(sender_id, location, x)
                    ...
                    if x["account_linking"]["status"] == "linked":
                        on_account_linked(
                            sender_id, x["account_linking"]["authorization_code"], x
                        )
    ...
    def set_greeting_text(self, greeting_text):
        ...
        @output:
            response_json
        """
        payload = {"setting_type": "greeting", "greeting": {"text": greeting_text}}
    ...
    def set_get_started_button(self, postback_text):
        ...
        payload = {
            ...
            "call_to_actions": [{"payload": postback_text}],
        }
        return self._send_payload(payload, self.request_endpoint_setting)
    ...
    def set_persistent_menu(self, buttons):
        ...
        return self._send_payload(payload, self.request_endpoint_messenger_profile)
    ...
    def send_text_message(self, recipient_id, text, quick_replies=[]):
        ...
        payload = {
            ...
            else {"text": text, "quick_replies": quick_replies},
        }
        return self._send_payload(payload, self.request_endpoint_message)

    def send_message_file_attachment(
        self, recipient_id, attachment_type, file_url, quick_replies=[]
    ):
        """
        @required:
            recipient_id
        ...
        @output:
            response_json
        """
        attachment = {"type": attachment_type, "payload": {"url": file_url}}
        return self._send_message(recipient_id, attachment, quick_replies)

    def send_message_generic(
        self, recipient_id, title, subtitle, image_url, buttons=[], quick_replies=[]
    ):
        """
        @required:
            recipient_id
        ...
        attachment = {
            ...
            "payload": {
                ...
                "elements": [
                    {"title": title, "image_url": image_url, "subtitle": subtitle}
                    if not buttons
                    else {
                        "title": title,
                        "image_url": image_url,
    ...
    def send_message_generic_carousel(
        self, recipient_id, element_list, quick_replies=[]
    ):
        element_array = []
        for each_element in element_list:
            element_array.append(
                {
                    "title": each_element["title"],
                    "image_url": each_element["image_url"],
                    "subtitle": each_element["subtitle"],
                }
                if not each_element["buttons"]
                else {
                    "title": each_element["title"],
                    "image_url": each_element["image_url"],
                    "subtitle": each_element["subtitle"],
                    "buttons": each_element["buttons"],
                }
            )
        attachment = {
            "type": "template",
            "payload": {"template_type": "generic", "elements": element_array},
        }
        from pprint import pprint
    ...
    def send_message_TypeC(self, recipient_id, fborder, quick_replies=[]):
        ...
        @output:
            response_json
        """
        attachment = {"type": "template", "payload": fborder.get_payload()}
        return self._send_message(recipient_id, attachment, quick_replies)
    ...
    def _send_message(self, recipient_id, attachment, quick_replies=[]):
        ...
        payload = {
            ...
            if not quick_replies
            else {"attachment": attachment, "quick_replies": quick_replies},
        }
        return self._send_payload(payload, self.request_endpoint_message)
    ...
    def _send_payload(self, payload, request_endpoint, request_type="post"):
        ...
        if request_type == "post":
            response = requests.post(
                request_endpoint, params=self.auth_args, json=payload, verify=False
            )
