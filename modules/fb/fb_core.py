...
class FBBot(object):
    def __init__(self, access_token, verify_token, **kwargs):
        ...
        self.request_endpoint_messenger_profile = '{0}/me/messenger_profile'.format(self.graph_url)
        ...
        if request.method == 'POST':
            ...
            for x in event:
                ...
                if x.get('message') and x['message'].get('text'):
                    message = 'empty' if (not ('message' in x) or not ('text' in x['message'])) else x['message'][
                        'text']
                    on_message_received(sender_id, message, x)
                elif x.get('message') and x['message'].get('attachments') and x['message']['attachments'][0].get(
                        'type') and x['message']['attachments'][0]['type'] == 'location':
    ...
    def send_message_generic_carousel(self, recipient_id, element_list, quick_replies=[]):
        element_array = []
        for each_element in element_list:
            element_array.append(
                ...
                } if not each_element["buttons"] else {
                    "title": each_element["title"],
                    "image_url": each_element["image_url"],
                    "subtitle": each_element["subtitle"],
