...
def make_flight_listing_req(
    dept_date, flight_number, access_code, board_point, off_point
):
...
def get_flight_staff_listing(date, board_point, off_point, flight_num):
    try:
        ...
        flight_listing = make_flight_listing_req(
            date, flight_num, access_code, board_point, off_point
        )
        if "status" in flight_listing and flight_listing["status"] is True:
            ...
            for index, pax in enumerate(pax_list):
                response_str += (
                    f"{index + 1}. {pax['fname']} {pax['lname']}. Priority: {pax['priority']} "
