...
def format_datetime(iso_time):
    ...
    return (
        new_datetime.strftime("%a %d %b, %Y") + " " + new_datetime.strftime("%I:%M%p")
    )
...
def get_flight_info(flight_number, iso_date):
    ...
    data = {"from_date": from_date, "to_date": to_date, "flight_number": flight_number}
...
def fetch_flight_status(df_request):
    ...
    response_dict = {"preamble": None, "response_list": []}
    try:
        ...
        if len(flight_info) == 0:
            response_dict[
                "preamble"
            ] = "I'm not seeing any flights for {flight_num} scheduled for {dept_date}".format(
                flight_num=flight_num, dept_date=day_of_date
            )
        else:
            for key, flight_resp in flight_info.items():
                ...
                arr_date_only = datetime.datetime.strptime(
                    flight_resp["arr_time_local"], "%Y-%m-%dT%H:%M:%S"
                ).date()
                dept_date_only = datetime.datetime.strptime(
                    flight_resp["dept_time_local"], "%Y-%m-%dT%H:%M:%S"
                ).date()
                ...
                if flight_status in ("Cancelled", "Delayed"):
                    flight_status = (
                        "cancellation" if flight_status == "Cancelled" else "delay"
                    )
                    msg = (
                        f"Unfortunately {flight_num} has suffered a {flight_status}. The estimated departure time "
                    ...
                    response_dict["response_list"].append(
                        {"flight_status": flight_status, "msg": msg, "fb_card": fb_card}
                    )
                elif flight_status == "Scheduled":
                    msg = (
                        "{flight_num} is scheduled to depart {dept_city} on {dept_time} ({dept_code} time) and arrive"
                    ...
                    response_dict["response_list"].append(
                        {"flight_status": flight_status, "msg": msg}
                    )
                    # fb_bot.send_text_message(user_session, msg)
                elif flight_status == "Completed":
                    msg = (
                        "{flight_num} has already reached {arr_city} from {dept_city}. It arrived on {arr_time} "
                    ...
                    msg = (
                        "{flight_num} departed {dept_city} at {dept_time} ({dept_code} time). It is currently"
                    ...
                    msg = (
                        f"{flight_num} has just landed in {flight_resp['arr_city']}. It arrived at "
                    ...
                    response_dict["response_list"].append(
                        {
                            "flight_status": "error",
                            "msg": "Sorry, I can't seem to "
                            "find any information for "
                            "that flight.",
                        }
                    )
        if len(response_dict["response_list"]) > 1:
            response_dict["preamble"] = (
                "It seems there were more than one flight {flight_num} on {dept_date}. "
                "This is usually because of overnight flights".format(
                    flight_num=flight_num, dept_date=day_of_date
                )
            )
        ...
        response_dict["response_list"].append(
            {
                "flight_status": "error",
                "msg": "Sorry, I can't seem to find any information for that flight.",
            }
        )
