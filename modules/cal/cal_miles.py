...
def query_miles_from_alms(member_id):
    ...
    member = member_type(
        memberId=member_id, retrieveMemberProfiles={"retrieveMemberProfileType": "ALL"}
    )
    header_type = client.get_element("ns0:header")
    header = header_type(userName=username, passWord=password)
    response = client.service.retrieveProfile(
        retrieveProfileRequest=member, _soapheaders=[header]
    )
