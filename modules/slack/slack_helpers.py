...
def create_flight_load_attachment_json(resp_arr, attachment_arr, channel_id, user_id):
    ...
    for attach, text_val in zip(attachment_arr, resp_arr):
        attach[0] = datetime.strptime(attach[0], "%d%m%y").strftime("%Y%m%d")  # Format the datetime
        output_arr.append(
            {
                "text": text_val,
                "channel": channel_id,
                    ...
                    "actions": [
                        {
                            ...
                            "type": "button",
                            "value": ' '.join(attach)  # The value that is passed when the user clicks in the button
                        }
                    ]
                }]
            }
